...
my $ex_ssr ={
    ...
    '2 рубля. Ученый-естествоиспытатель М.В. Ломоносов, к 300-летию со дня рождения' => '-', 
    ...
    '2 рубля. Писатель И.А. Гончаров - 200-летие со дня рождения' => '-',
    # гагара 2012
    # усач 2012
    '2 рубля. Забайкальский солонгой' => '-',

    '2 рубля. Сметанина Р.П. Выдающиеся спортсмены России (Лыжные гонки)' => '-',
    '2 рубля. В.С. Черномырдин, 75-летие со дня рождения' => '-',
    '2 рубля. Кулакова Г.А. Выдающиеся спортсмены России (Лыжные гонки)' => '-', 
    # штаб 2013 
...
foreach my $lot (@lots) {
    ...
    }elsif ($ex_sr->{$title}->{$lot_year} && ($mint!~/$ex_sr->{$title}->{$lot_year}/) && $price<10000 && ($metal eq 'Ag') && $lot_condition!~/(AU|MS|PL|UNC) (\d+|Det)/) {
        $found_count++;
